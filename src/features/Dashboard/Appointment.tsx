...
const Appointment: React.FC<IProps> = ({ setLoadingExport, setLinkExport, setIsExport }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const resp = await DashBoardService.getAppointments(MERCHANT_TYPE, timeRange, custom);
    ...
    const handleExport = async () => {
        try {
            ...
            const link = await DashBoardService.exportAppointments(MERCHANT_TYPE, timeRange, custom);
            setLinkExport(link);
            setIsExport(true);
            ...
            <Spin spinning={loading}>
                <div className="flex justify-between flex-wrap items-start">
                    <div>
                        ...
                        <div className="mt-1.75 text-davy-grey text-3.75 font-semibold mr-2">Appointment Total</div>
                    </div>
                                            ...
                                            <div className="flex">
                                                <div
                                                    className={classNames("h-4 w-4 rounded mb-2", {
                                                        "bg-green-primary":
                                                            item?.name === "POS" || item?.name === "Store",
                                                        "bg-green-secondary":
                                                            item?.name === "Website/App" || item?.name === "Website",
                                                        "bg-yellow-primary":
                                                            item?.name === "Other" || item?.name === "CallOrder",
                                                    })}
                                                />
