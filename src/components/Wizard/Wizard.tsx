...
const Wizard: React.FC<IProps> = ({ steps = [], current = 0 }) => {
    return (
        <div className={`w-full flex flex-wrap items-center my-5`}>
            {steps.map((step, index) => {
                ...
                return (
                    <div className={classNames("h-20 relative  border-b border-white md:border-0", width)} key={index}>
                        <div
                            className={classNames(
                                "w-full h-full text-center pr-8 flex flex-col justify-center text-white",
                                bgRec
                            )}
                        >
              ...
              border-t-[40px] border-b-transparent border-t-transparent border-l-[30px]`,
                    ...
                    border-b-transparent border-t-transparent border-l-white border-l-[30px]"
                        ></div>
                        <div
                            className={classNames(`hidden md:block w-[30px] h-20 absolute top-0 right-0 z-0`, bg)}
                        ></div>
                    </div>
                );
            })}
